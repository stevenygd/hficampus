...
/**
 * Get the attributes of File in Storage
 * 获取Storage内文件的属性
 *
 * @access	public
 * @return	array
 */
if ( ! function_exists('s_get_attr'))
{
	function s_get_attr($filepath, $attrKey = array())
	{
		$_s = new SaeStorage();  //初始化Storage对象
		$_f = _s_get_path($filepath);
		return $_s->getAttr($_f['domain'], $_f['filename'], $attrKey);
	}
}
